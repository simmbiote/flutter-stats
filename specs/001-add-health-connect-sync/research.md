...
| `health` and its underlying Health Connect SDK are evolving and currently use an alpha client. | Pin versions, review the resolved lockfile/Gradle dependencies, run an early build smoke test, keep the adapter thin, and retain a native-bridge contingency. |
